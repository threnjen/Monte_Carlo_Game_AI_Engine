...
class GameEngine():
    ...
    def play_game_byturns(self, simulations):
        ...
        current_player = self.game.get_legal_actions(policy=False)[
            1]  # Get starting player

        self.montecarlo = MonteCarloEngine(
            current_player)  # initiate the monte carlo engine
        ...
        while not self.game.is_game_over():
            ...
            print("Game gets " + str(self.sims_this_turn) +
                  ' simulations for this turn.')
            print("Player's turn: Player " +
                  str(current_player))  #whose turn is it?

            # Toggle the following block for heavy reporting
            if self.verbose:
                try:
                    print("Entry state")
                    print(self.current_node.depth,
                          self.current_node.node_action,
                          self.current_node.player_owner, self.current_node)
            ...
            current_player = self.game.get_legal_actions()[
