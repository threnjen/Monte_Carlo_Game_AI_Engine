...
class Game_Engine():
    ...
    def __init__(self):    
        ...
        self.root = MonteCarlo(state=self.board._state, parent=None, parent_action=None, legal_actions=self.board.get_legal_actions()) # instantiate the root node of the game
    ...
    def expand(self, current_node):
        ...
        next_state = self.board.update_game(action) # calls move function on the action to get next_state. Calls MOVE in GAME_LOGIC object
    ...
    def rollout(self):
        ...
        while not self.board.is_game_over(): # checks the state for game over boolean and loops if it's false
            ...
            self.board.update_game(action) # takes action just pulled at random. Calls MOVE in GAME_LOGIC object
...
class SimpleArrayGame():
    ...
    def game_result(self):
        ...
        '''
        if self._state[0]==1:
            if self._state[1]==1 or self._state[2]==1:             
                print("Score is 1")
                return 1
        if self._state[1]==1:
            if self._state[0]==1 or self._state[2]==1:
                print("Score is -1")
                return -1
        if self._state[2]==1:
            if self._state[0]==1 or self._state[1]==1:
                print("Score is 0")
                return 0    
...
game.play_game(1)
    ...
    #    return self.board.is_game_over() # checks is_game_over for boolean. Calls IS_GAME_OVER (defined by user)
    #    #return False    
